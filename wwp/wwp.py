...
def modifyPrime(operation, operand, stop, start=1):
    '''
    ...
    Returns:
        list: A list of prime numbers within the specified range after applying the specified 'operation' and 'operand'.
        None: If 'start' or 'stop' is less than or equal to 0.
    '''
...
def graphPrimes(stop, operation="+", operand=0, start=1):
    ...
    if stop<=0 or start<=0:
        return None
    primes = np.array(modifyPrime(operation, operand, stop))
    primes2 = np.array(firstNPrimes(stop))
    if primes[0] == primes2[0]:
        plt.plot(primes2, color='red', label='Parent Prime')
    else:
        plt.plot(primes2, color='red', label='Parent Prime')
        plt.plot(primes, color='blue', label='Modified Prime')
    plt.xlabel('Number Line')
